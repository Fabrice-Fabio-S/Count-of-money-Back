  ...
  googleCallback: async (req, res) => {
    passport.authenticate(
      ...
      (err, response) => {
        ...
        console.log(
          "step-3 : " + response.displayName + " " + response.emails[0].value
        );
        ...
        const firstname = response.name.givenName;
        const lastname = response.name.familyName;
        ...
        UserModel.setCryptoList(userId, crypto)
          .then((result) => {
            ...
            return Utils.getJsonResponse(
              200,
              "",
              { message: "Crypto list has been update" },
              res
            );
          })
          .catch((err) => {
